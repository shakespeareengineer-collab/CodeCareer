# Coding Career Overview

## Known Languages (Most - Least favorite)
1. Python  
    ```Python
    def greet():
        string1 = "This is Python"
        print(string1)
    greet()
    ```
    2. Java
    ```Java
    void greet(){
        String string1 = "This is Java";
        System.out.println(string1);
    }
    greet();
    ```
3. HTML
    ```html
    <!DOCTYPE html>
    <html>
      <head>
        <title>Example</title>
      </head>
      <body>
        <h1>Hello from HTML</h1>
        <p>This is an HTML code block.</p>
      </body>
    </html>
    ```
    - Javascript
    ```javascript
    function greet() {
      const string1 = "This is JavaScript";
      console.log(string1);
    }
    greet();
    ```
    - CSS
    ```css
    body {
      font-family: Arial, sans-serif;
      background-color: #f4f4f4;
      color: #333;
    }
    ```
4. Markdown
    ```Markdown
    # This is Markdown
    ## This is Markdown
    ### This is Markdown
    ...
    ###### This is the smallest Markdown header
    ```
5. C#
    ```csharp
    using System;

    class Program
    {
        static void Main()
        {
            string string1 = "This is C#";
            Console.WriteLine(string1);
        }
    }
    ```
## Successful Projects
+ [Platform](Platform.py)  
    Python demo file of `pygame` as a platform game window.

+ SpaceDodge
    Available on codepad. Player rectangle dodges stars as it flies through space. Made by `BoredBoardCoder`.

+ [TKinter GUI](tkinter_gui.py)  
    Python demo file of `tkinter` as a simple "Click me!" window.

+ [SimpleHTMLPage.com](https://raw.githubusercontent.com/shakespeareengineer-collab/CodeCareer/main/index.html)  
    A very simple trustworthy website, has no info and doesn't actually collect any info.

## Favorite Coding Apps
- [VS Code](https://code.visualstudio.com) (compatible with most languages)
- [Codepad](https://codepad.app) (use in browser)
- Notepad (no code re-directs)

## Notes
>I quite like Python language because of how simple and easy it is, but I also like Java the same amount, because of how un-simple it is. Also I've slowly forgotten C# and all I remember is in the last block above.
